...
use crate::utils::{remove_comment, remove_comment_multiple_lines, FileBuf, GggError};
...
impl Display for I2SVersionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Unknown I2S version, '{}'. Available versions are: '2014', '2020'.",
            self.given
        )
    }
}
...
pub fn iter_i2s_header_params(
    i2s_input_file: &Path,
    i2s_version: I2SVersion,
) -> Result<I2SParamIter, GggError> {
...
/// - [`iter_i2s_lines`] if all lines (including comments) from an input file are required.
pub fn iter_i2s_header_params_with_number(
    i2s_input_file: &Path,
    i2s_version: I2SVersion,
) -> Result<I2SParamIterWithIndex, GggError> {
...
pub fn iter_i2s_lines(
    i2s_input_file: &Path,
    i2s_version: I2SVersion,
) -> Result<I2SLineIter<FileBuf<BufReader<File>>>, GggError> {
    let file = FileBuf::open(i2s_input_file)?;
...
impl I2SParamIter {
    ...
    pub fn new(
        i2s_reader: FileBuf<BufReader<File>>,
        i2s_version: I2SVersion,
        all_params: bool,
    ) -> Self {
        Self {
...
pub struct I2SLineIter<R: BufRead> {
    file: R,
    curr_param: usize,
    i2s_version: I2SVersion,
}

impl<R: BufRead> I2SLineIter<R> {
    /// Create a new instance of the iterator.
    ///
    ...
    pub fn new(i2s_reader: R, i2s_version: I2SVersion) -> Self {
        Self {
            file: i2s_reader,
            curr_param: 0,
            i2s_version,
...
impl<R: BufRead> Iterator for I2SLineIter<R> {
...
fn iter_i2s_lines_inner<R: BufRead>(
    mut file: R,
    curr_param: usize,
    param_num_lines: usize,
    max_n_param: Option<usize>,
) -> Option<std::io::Result<(bool, String)>> {
            ...
            Ok(0) => {
                if n_param_lines_read == 0 {
                    ...
                    return Some(Err(std::io::Error::other(
                        "File ended midway through reading a parameter.",
                    )));
                }
            }
...
impl I2SInputModifcations {
    ...
    pub fn set_parameter_change(&mut self, param_num: usize, new_value: String) {
        ...
        self.header.push(I2SHeaderEdit {
            parameter: param_num,
            value: new_value,
        });
    }
    ...
    pub fn example_json_string(pretty: bool) -> String {
        let example = Self {
            header: vec![
                I2SHeaderEdit {
                    parameter: 1,
                    value: "./igms".to_string(),
                },
                I2SHeaderEdit {
                    parameter: 2,
                    value: "./spectra".to_string(),
                },
                I2SHeaderEdit {
                    parameter: 7,
                    value: "1 1".to_string(),
...
impl From<Vec<I2SHeaderEdit>> for I2SInputModifcations {
    fn from(value: Vec<I2SHeaderEdit>) -> Self {
        Self { header: value }
    }
}
...
pub fn write_opus_catalogue_table<W: std::io::Write>(
    writer: &mut W,
...
pub struct OpusCatalogueEntry {
    ...
    longitude: f64,
    altitude: f64,
    instrument_temperature: f64,
    instrument_pressure: f64,
    instrument_humidity: f64,
    outside_temperature: f64,
    outside_pressure: f64,
    outside_humidity: f64,
    solar_intensity_average: f64,
    fractional_variation_solar_intensity: f64,
    wind_speed: f64,
    wind_direction: f64,
}
...
pub struct OpusCatalogueEntryBuilder {
    ...
    altitude: Option<f64>,
    instrument_temperature: Option<f64>,
    instrument_pressure: Option<f64>,
    instrument_humidity: Option<f64>,
    outside_temperature: Option<f64>,
    outside_pressure: Option<f64>,
    outside_humidity: Option<f64>,
    solar_intensity_average: Option<f64>,
    fractional_variation_solar_intensity: Option<f64>,
    wind_speed: Option<f64>,
    wind_direction: Option<f64>,
}

impl OpusCatalogueEntryBuilder {
    pub fn finalize(self, fill_value: f64) -> CatalogueEntryResult<OpusCatalogueEntry> {
        Ok(OpusCatalogueEntry {
            spectrum_name: self.spectrum_name,
            year: self
                .year
                .ok_or_else(|| CatalogueEntryError::MissingRequiredField("year"))?,
            month: self
                .month
                .ok_or_else(|| CatalogueEntryError::MissingRequiredField("month"))?,
            day: self
                .day
                .ok_or_else(|| CatalogueEntryError::MissingRequiredField("day"))?,
            run: self
                .run
                .ok_or_else(|| CatalogueEntryError::MissingRequiredField("run"))?,
            ...
            fractional_variation_solar_intensity: self
    ...
    pub fn with_time(
        ...
        day: u32,
        run: u32,
    ) -> CatalogueEntryResult<Self> {
        if NaiveDate::from_ymd_opt(year, month, day).is_none() {
            return Err(CatalogueEntryError::InvalidValue {
    ...
    pub fn with_coordinates(
        ...
        if latitude < -90.0 || latitude > 90.0 {
            return Err(CatalogueEntryError::InvalidValue {
                field: "latitude",
                reason: format!("Latitude must be between -90 and +90, {latitude} is invalid"),
            });
        } else {
        ...
        if longitude < -180.0 || longitude > 360.0 {
            return Err(CatalogueEntryError::InvalidValue {
                field: "longitude",
                reason: format!("Longitude must be between -180 and +360, {longitude} is invalid"),
            });
    ...
    pub fn with_instrument(
        mut self,
        instr_temperature_degC: f64,
        instr_pressure_hPa: f64,
        instr_humidity_percent: f64,
    ) -> Self {
    ...
    pub fn with_solar(mut self, sia: f64, fvsi: f64) -> Self {
        self.solar_intensity_average = Some(sia);
        self.fractional_variation_solar_intensity = Some(fvsi);
        self
    }

    pub fn with_wind(mut self, wind_speed: f64, wind_direction: f64) -> Self {
