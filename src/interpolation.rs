...
pub enum InterpolationError {
    ...
    #[error("Input arrays were too short, needed at least {req_len} elements but got only {actual_len}")]
    InputTooShort{req_len: usize, actual_len: usize},
    #[error("Requested output coordinate ({out}) falls outside the input domain ({left} to {right})")]
    OutOfDomain{left: String, right: String, out: String}
}

pub trait InterpolationMethod {
    fn interp1d<F: Float + Debug>(&self, input_x: &[F], input_y: &[F], output_x: F) -> Result<F, InterpolationError>;

    fn interp1d_to_time<Z: TimeZone, F: Float + Debug>(&self, input_t: &[DateTime<Z>], input_y: &[F], output_t: DateTime<Z>) -> Result<F, InterpolationError> {
        let input_x = input_t.iter()
            .map(datetime_to_float)
            .collect_vec();
    ...
    fn check_1d_inputs<F: Float + Debug>(&self, input_x: &[F], input_y: &[F], output_x: F, must_be_in_bounds: bool, min_len: usize) -> Result<(), InterpolationError> {
        if input_x.len() != input_y.len() {
            return Err(InterpolationError::InputLengthMismatch { x_len: input_x.len(), y_len: input_y.len() });
        }

        // Now we know both are the same length, so only need to test 1
        if input_x.len() < min_len {
            return Err(InterpolationError::InputTooShort { req_len: min_len, actual_len: input_x.len() });
        }
        ...
        if must_be_in_bounds {
            ...
            let right_bound = input_x.iter().copied().reduce(F::max).expect("input_x.max() should return Some if input_x.min() returned Some()");

            if output_x < left_bound || output_x > right_bound {
                return Err(InterpolationError::OutOfDomain { left: format!("{left_bound:?}"), right: format!("{right_bound:?}"), out: format!("{output_x:?}") });
            }
        }
...
fn datetime_to_float<Z: TimeZone, F: Float>(t: &DateTime<Z>) -> F {
    let ts = F::from(t.timestamp()).expect("Expected to be able to convert timestamp i64 to output float type");
    let ts_frac = F::from(t.timestamp_subsec_nanos()).expect("Expected to be able to convert timestamp subsecond nanoseconds u32 to output float type");
    ts + ts_frac / F::from(1e9).unwrap()
}
...
mod tests {
    ...
    fn test_constant_value_error_checks() {
        ...
        match err {
            InterpolationError::InputTooShort { req_len, actual_len } => {
        ...
        let err = interpolator.interp1d(&[1.0, 2.0], &[2.0, 4.0], 0.0).unwrap_err();
        match err {
            InterpolationError::OutOfDomain { left: _, right: _, out: _ } => (),
    ...
    fn test_constant_value_no_extrap() {
        ...
        let y_out = interpolator.interp1d(x.as_slice(), y.as_slice(), 1.25).unwrap();
    ...
    fn test_constant_value_with_extrap() {
        ...
        let y_out = interpolator.interp1d(x.as_slice(), y.as_slice(), 10.0).unwrap();
    ...
    fn make_test_datetimes() -> [DateTime<Utc>; 3] {
        let fmt = "%Y-%m-%d %H:%M";
        [
            NaiveDateTime::parse_from_str("2023-08-26 09:00", fmt).unwrap().and_local_timezone(Utc).unwrap(),
            NaiveDateTime::parse_from_str("2023-08-26 09:05", fmt).unwrap().and_local_timezone(Utc).unwrap(),
            NaiveDateTime::parse_from_str("2023-08-26 09:10", fmt).unwrap().and_local_timezone(Utc).unwrap(),
        ]
    }
}