...
impl FromStr for ColInputData {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let re = INPUT_MD5_REGEX
            .get_or_init(|| regex::Regex::new("(?<md5>[0-9a-fA-F]{32})  (?<path>.+)").unwrap());

        let s = s.trim();
        let caps = re.captures(s).ok_or_else(|| HeaderError::ParseError {
            location: s.into(),
            cause:
                "Did not match expected format of 32 hex digit checksum, two spaces, then a path"
                    .to_string(),
        })?;
...
pub fn get_col_files(multiggg_file: &Path, run_dir: &Path) -> Result<Vec<PathBuf>, HeaderError> {
    let col_file_basenames =
        utils::get_windows_from_multiggg(multiggg_file, true).map_err(|e| {
            HeaderError::custom(format!("could not get windows from multiggg file: {e}"))
        })?;
    ...
    if missing_files.is_empty() {
        ...
        let msg = format!(
            "Missing {} of {} expected .col files, missing windows were: {missing_str}",
            missing_files.len(),
            nwin
        );
...
pub fn get_all_col_files(run_dir: &Path) -> error_stack::Result<Vec<PathBuf>, GggError> {
    let mut col_files = vec![];
    let res = run_dir.read_dir().change_context_lazy(|| {
        GggError::custom(format!(
            "error occurred while listing directory contents of {}",
            run_dir.display()
        ))
    })?;
    for entry_res in res {
        let entry = entry_res.change_context_lazy(|| {
            GggError::custom(format!(
                "error occurred while getting information about a directory entry in {}",
                run_dir.display()
            ))
        })?;
...
pub fn get_file_from_col_header<F>(
    col_files: &[PathBuf],
    run_dir: &Path,
    get_file: F,
) -> Result<PathBuf, HeaderError>
where
    F: Fn(ColFileHeader) -> PathBuf,
{
    if col_files.is_empty() {
        return Err(HeaderError::custom("no .col files found"));
    }

    let mut fbuf = FileBuf::open(&col_files[0]).map_err(|e| HeaderError::CouldNotRead {
        location: col_files[0].clone().into(),
        cause: e.to_string(),
    })?;

    let first_header = read_col_file_header(&mut fbuf).map_err(|e| HeaderError::CouldNotRead {
        location: col_files[0].clone().into(),
    ...
    for cfile in &col_files[1..] {
        let mut fbuf = FileBuf::open(cfile).map_err(|e| HeaderError::CouldNotRead {
            location: cfile.to_path_buf().into(),
            cause: format!("could not open this .col file: {e}"),
        })?;
        let header = read_col_file_header(&mut fbuf).map_err(|e| HeaderError::CouldNotRead {
            location: cfile.to_path_buf().into(),
        ...
        if new_file != expected_file {
            return Err(HeaderError::custom(format!(
                "mismatched files in .col header: {} gave {}, while {} gave {}",
                (&col_files[0]).display(),
...
pub fn get_runlog_from_col_files(
    multiggg_file: &Path,
    run_dir: &Path,
) -> Result<PathBuf, HeaderError> {
...
pub fn read_col_file_header<F: BufRead>(
    file: &mut FileBuf<F>,
) -> error_stack::Result<ColFileHeader, HeaderError> {
    let (nhead, ncol) =
        utils::get_nhead_ncol(file).change_context_lazy(|| HeaderError::ParseError {
            location: file.path.as_path().into(),
            cause: "Could not parse number of header lines and data columns".to_string(),
        })?;

    if nhead != 21 {
        error_stack::bail!(HeaderError::NumLinesMismatch {
            expected: 21,
            got: nhead
        });
    }
...
pub fn open_and_iter_col_file(path: &Path) -> error_stack::Result<ColRowIter, BodyError> {
    ...
    let header = read_col_file_header(&mut fbuf).change_context_lazy(|| {
        BodyError::could_not_read(
            "error getting information from .col file header",
            Some(path.into()),
            None,
            None,
        )
    ...
    let fmt = fortformat::FortFormat::parse(&header.format).map_err(|e| {
        BodyError::unexpected_format(
            format!("unable to parse Fortran format spec: {e}"),
            Some(path.into()),
            None,
            None,
        )
    ...
    let (_, ret_gases) = header.command_line.split_once(':').ok_or_else(|| {
        BodyError::unexpected_format(
            "command line is expected to have one colon",
            Some(path.into()),
            None,
            Some(header.command_line.clone()),
        )
    })?;

    let primary_gas = ret_gases
        .split_whitespace()
        .next()
        .ok_or_else(|| {
            BodyError::unexpected_format(
