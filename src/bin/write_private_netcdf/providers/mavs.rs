...
impl MavFile {
    pub(crate) fn new(mav_file_path: PathBuf) -> error_stack::Result<Self, CliError> {
        let first_mav_block = open_and_iter_mav_file(mav_file_path.clone())
            ...
            .change_context_lazy(|| {
                CliError::input_error("failed to read the first block from the .mav file")
            })?;

        let height = first_mav_block.data.get("Height").ok_or_else(|| {
            CliError::input_error("did not find a column named 'Height' in the first .mav block")
        })?;
        let ncell = height.mapv(|z| if z < -1.0 { 1 } else { 0 }).sum() as usize;
        let nlev = height.len() - ncell;
        Ok(Self {
            mav_file_path,
            mav_columns: first_mav_block.column_order,
            ncell,
            nlev,
        })
    }
    ...
    fn build_mav_arrays(
        ...
        let mut atmosphere = HashMap::from_iter(self.mav_columns.iter().map(|c| {
            (
                c.to_string(),
                Array2::from_elem((ntimes, self.nlev), f32::NAN),
            )
        }));
        let mut cells = HashMap::from_iter(self.mav_columns.iter().map(|c| {
            (
                c.to_string(),
                Array2::from_elem((ntimes, self.ncell), f32::NAN),
            )
        }));
        ...
        let mut curr_block = mav_iter
            .next()
            .ok_or_else(|| WriteError::custom(".mav file is empty"))?
            .change_context_lazy(|| WriteError::file_read_error(&self.mav_file_path))?;
        let mut next_block_opt = mav_iter
            .next()
            .transpose()
            .change_context_lazy(|| WriteError::file_read_error(&self.mav_file_path))?;

        // The altitude vector should not change, so we just get it from the first block
        let atmosphere_alts = curr_block
            .data
            .get("Height")
            .ok_or_else(|| {
                WriteError::custom("Expected the first .mav block to contain a 'Height' column")
            })?
            .slice(s![self.ncell..])
            .to_owned();
        ...
        loop {
            ...
            for (colname, arr) in curr_block.data.iter() {
                ...
                for i_time in i_start_time..i_end_time {
                    ...
                    atmosphere
                        .get_mut(colname)
                        .expect(
                            "all .mav file columns should have an array in the atmosphere hashmap",
                        )
                        .slice_mut(s![i_time, ..])
                        .assign(&atm_values);
                }
            ...
            if let Some(next_block) = next_block_opt {
                ...
                next_block_opt = mav_iter
                    .next()
                    .transpose()
                    .change_context_lazy(|| WriteError::file_read_error(&self.mav_file_path))?;
            } else {
                return Ok(MavArrays {
                    height: atmosphere_alts,
                    cell_profs: cells,
                    atm_profs: atmosphere,
                    col_order: curr_block.column_order,
                });
            }
        }
    }
...
impl DataProvider for MavFile {
    ...
    fn write_data_to_nc(
        ...
        spec_indexer: &crate::interface::SpectrumIndexer,
        writer: &dyn crate::interface::GroupWriter,
        group_selector: &dyn crate::interface::GroupSelector,
        pb: indicatif::ProgressBar,
    ) -> error_stack::Result<(), crate::errors::WriteError> {
        let mav_data = self.build_mav_arrays(spec_indexer, writer)?;

        let mav_basename = self
            .mav_file_path
            .file_name()
            .expect("Couldn't get the basename of the .mav file")
            .to_string_lossy()
            .to_string();
        ...
        let mut variables: Vec<Box<dyn VarToBe>> =
            vec![Box::new(ConcreteVarToBe::new_with_checksum(
                "prior_altitude",
