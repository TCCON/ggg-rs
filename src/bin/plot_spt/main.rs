...
impl SptHeader {
    fn from_header_line(line: &str) -> Result<SptHeader, HeaderError> {
        fn parse_field<T: FromStr>(s: &str, i: usize, full_line: &str) -> Result<T, HeaderError> {
            s.parse::<T>().map_err(|_| HeaderError::ParseError {
                location: full_line.into(),
                cause: format!("Could not parse {}th element {} as a number", i + 1, s),
            })
        }
...
fn read_spt_file(spt_file: &Path) -> Result<SptData, GggError> {
    ...
    if col_names.len() != ncol {
        return Err(HeaderError::NumColMismatch {
            location: spt_file.into(),
            got: col_names.len(),
            expected: ncol,
        }
        .into());
    ...
    for (iline, line) in fbuf.into_reader().lines().enumerate() {
        ...
        if split.len() < ncol {
            return Err(GggError::DataError {
                path: spt_file.to_owned(),
                cause: format!(
                    "Line {} does not have the expected number of columns ({})",
                    iline + nhead,
                    ncol
                ),
            });
...
fn main() -> Result<(), GggError> {
    ...
    let spt_basename = clargs
        .spt_file
        .file_name()
        .expect("Expecting input SPT file to have a path component after the final slash");
    ...
    let freq = spt.data.remove("Freq").ok_or_else(|| GggError::DataError {
        path: clargs.spt_file.clone(),
        cause: "Could not find the 'Freq' column".to_owned(),
    })?;
    ...
    if clargs.columns.contains("Tm") {
        ...
        let trace = Scatter::new(freq.clone(), tm)
            .name("Measured")
            .mode(Mode::Lines);
    ...
    if clargs.columns.contains("Tc") {
        let tc = spt
            .data
            .remove("Tc")
            .expect("Did not find the Tc column in the spt file");
        let trace = Scatter::new(freq.clone(), tc)
            .name("Total calc.")
            .mode(Mode::Lines);
    ...
    for key in spt.columns.iter() {
        ...
        if let Some(value) = spt.data.remove(key) {
            let trace = Scatter::new(freq.clone(), value)
                .name(key)
                .mode(Mode::Lines);
            plot.add_trace(trace);
        }
