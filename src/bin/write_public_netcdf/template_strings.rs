...
impl TokenError {
    ...
    fn extra_arg<S: Into<String>, V: Into<String>>(
        expected_number: usize,
        function: S,
        arg_value: V,
        span: pest::Span,
...
pub fn apply_template_transformations(
    ...
    let parsed_str = TemplateStringParser::parse(Rule::attribute, template_str)
        .unwrap()
        .next()
        .unwrap(); // getting the attribute rule should never fail
...
fn handle_replacement(
    ...
    for part in replacement {
        match part.as_rule() {
            Rule::identifier => {
                return get_id_arg(Some(part), private_name, public_name).map(|s| s.to_string());
            }
            Rule::function => {
                ...
                match fxn_name.as_str() {
                    ...
                    _ => {
                        return Err(TokenError::unknown_function(
                            fxn_name.as_str(),
                            fxn_name.as_span(),
                        ))
                    }
...
fn get_id_arg<'a>(
    ...
    match value.as_rule() {
        Rule::identifier => {}
        _ => {
            return Err(TokenError::wrong_token(
                "identifier (e.g. name or pubname)",
                &value,
            ))
        }
    }
...
fn get_string_arg<'a>(arg: Option<Pair<'a, Rule>>, arg_num: usize) -> Result<&'a str, TokenError> {
    let arg = arg.ok_or_else(|| TokenError::missing_arg(arg_num))?;
...
fn check_end_of_args(
    arg: Option<Pair<'_, Rule>>,
    arg_num: usize,
    function: &str,
) -> Result<(), TokenError> {
    if let Some(a) = arg {
        Err(TokenError::extra_arg(
            arg_num,
            function,
            a.as_str(),
            a.as_span(),
...
fn transform_map<'a>(
    original: &'a str,
    map_key: &str,
    all_maps: &'a HashMap<String, HashMap<String, String>>,
) -> Option<&'a str> {
...
mod tests {
    ...
    fn test_parsing_wrong_arg_type() {
        let err =
            apply_template_transformations("{upper('oops')}", Some("xco2"), None, &HashMap::new())
                .unwrap_err();

        if let TokenError::WrongToken {
            expected,
            got,
            index,
        } = err
    ...
    fn test_parsing_no_public() {
        let err = apply_template_transformations("{pubname}", Some("xco2"), None, &HashMap::new())
            .unwrap_err();
    ...
    fn test_parsing_regex_replacements() {
        ...
        assert_eq!(
            &s,
            "Col variable - h2o original vertical column in the o2_7885 window"
        );
    }
    ...
    fn test_parsing_map_replacements() {
        ...
        let mapping = HashMap::from_iter([("species".to_string(), inner_map)]);
        ...
        assert_eq!(
            &s,
            "column average mole fraction of carbon dioxide in parts per million"
        );
