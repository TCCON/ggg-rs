...
use crate::{constants::PRIOR_INDEX_VARNAME, copying::{copy_utils::chars_to_string, find_subset_dim}, TIME_DIM_NAME};
...
const GEOS_FLAGS: [i8; 4] = [FPIT_MET_FPIT_CHM, IT_MET_IT_CHM, FPIT_MET_IT_CHM, NON_STD_PRIORS];
...
pub(super) fn add_geos_version_variable(
    ...
    let mut var = public_file.add_variable::<i8>(public_varname, &[TIME_DIM_NAME])
        .change_context_lazy(|| CopyError::context(format!("creating GEOS version variable, '{public_varname}'")))?;
    var.set_fill_value(i8::MIN)
        .change_context_lazy(|| CopyError::context(format!("setting fill value for GEOS version variable, '{public_varname}'")))?;


    // Attributes
    var.put_attribute(
        "long_name",
        "a priori data source"
    ...
    var.put_attribute(
        "flag_values",
        Vec::from_iter(GEOS_FLAGS.iter().copied())
    ).change_context_lazy(|| CopyError::context(format!(
    ...
    var.put(source_flags.view(), Extents::All)
        .change_context_lazy(|| CopyError::context(format!("writing data to GEOS version variable '{public_varname}'")))?;
...
fn make_geos_version_array(
    ...
    let met3d_versions = get_geos_version_strings(private_file, "met3d", prior_index_varname, time_subsetter)?;
    let met2d_versions = get_geos_version_strings(private_file, "met2d", prior_index_varname, time_subsetter)?;
    let chm3d_versions = get_geos_version_strings(private_file, "chm3d", prior_index_varname, time_subsetter)?;

    let n = if met2d_versions.len() != met3d_versions.len() {
        return Err(CopyError::custom("GEOS met2d and met3d version variables do not have the same length").into())
    } else if met3d_versions.len() != chm3d_versions.len() {
        return Err(CopyError::custom("GEOS met3d and chm3d version variables do not have the same length").into())
    } else {
        met2d_versions.len()
    };
...
fn get_geos_version_strings(
    ...
    let var = private_file.variable(&varname)
        .ok_or_else(|| CopyError::MissingReqVar(varname.clone()))?;
    let chars = var.get::<NcChar, _>(Extents::All)
        .change_context_lazy(|| CopyError::context(format!("getting data from variable '{varname}'")))?;
    let chars = chars.into_dimensionality::<Ix2>()
        .change_context_lazy(|| CopyError::context(format!("converting '{varname}' into a 2D array")))?;
    ...
    let prior_index = prior_index_var
        ...
        .change_context_lazy(|| CopyError::context(format!("getting data from variable '{prior_index_varname}'")))?;
    ...
    let prior_index = prior_index.into_dimensionality::<Ix1>()
        .change_context_lazy(|| CopyError::context(format!("converting '{prior_index_varname}' into a 1D array")))?
        .mapv(|v| v as usize);


    let chars = nc_utils::expand_priors(chars.view(), prior_index.view())
        .change_context_lazy(|| CopyError::context(format!("expanding the variable '{varname}'")))?;

    let versions = chars.rows()
        .into_iter()
