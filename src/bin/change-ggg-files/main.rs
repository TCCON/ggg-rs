use std::{ffi::OsString, io::{BufRead, BufReader, Write}, os::unix::ffi::OsStrExt, path::{Path, PathBuf}, process::ExitCode};
...
impl Cli {
    ...
    fn no_op(&self) -> bool {
        if self.ak_output_pattern.is_some() { return false;}
        if self.ak_output_limit.is_some() { return false;}
        if self.spt_output_pattern.is_some() { return false;}
        if self.spt_output_limit.is_some() { return false;}
...
fn modify_ggg_file(ggg_file: &Path, args: &Cli) -> error_stack::Result<(), CliError> {
    let f = std::fs::File::open(ggg_file)
        .change_context_lazy(|| CliError::IoError)?;
    let f = BufReader::new(f);
    let lines: Vec<String> = f.lines().try_collect()
        .change_context_lazy(|| CliError::IoError)
        .attach_printable_lazy(|| format!("Could not read lines from {}", ggg_file.display()))?;
    
    let mut out = std::fs::File::create(ggg_file)
        .change_context_lazy(|| CliError::IoError)?;
    ...
    let window = ggg_file.file_name()
        .ok_or_else(|| CliError::PathError(ggg_file.to_path_buf(), "Could not get file basename".to_string()))?
        .as_bytes()
        .split(|b| b == &b'.')
        .next()
        .ok_or_else(|| CliError::PathError(ggg_file.to_path_buf(), "Cannot get window from .ggg file name".to_string()))?;
    let window = String::from_utf8(window.to_owned())
        .change_context_lazy(|| CliError::PathError(ggg_file.to_path_buf(), "Window name in .ggg file contains invalid unicode".to_string()))?;


    for (i, line) in lines.into_iter().enumerate() {
        let new_line = if i == 14 {
            // AK line
            make_output_line(&window, &line, args.ak_output_pattern.as_deref(), args.ak_output_limit, args.make_output_dirs)
                .change_context_lazy(|| CliError::InFile(ggg_file.to_path_buf()))?
        } else if i == 15 {
            // Spectral fit line
            make_output_line(&window, &line, args.spt_output_pattern.as_deref(), args.spt_output_limit, args.make_output_dirs)
                .change_context_lazy(|| CliError::InFile(ggg_file.to_path_buf()))?
        } else {
            line
        };
        ...
        writeln!(&mut out, "{new_line}")
                .change_context_lazy(|| CliError::IoError)
                .attach_printable_lazy(|| format!("Failed while writing line {} of {}", i + 1, ggg_file.display()))?;
    }

    Ok(())
}

fn make_output_line(window: &str, orig_line: &str, output_pattern: Option<&str>, max_num_file: Option<u64>, mkdir: bool) -> error_stack::Result<String, CliError> {
    let mut orig_parts = orig_line.split_ascii_whitespace();
    let orig_path = orig_parts.next()
        .ok_or_else(|| CliError::FileFormatError("expected the original AK/SPT line to have an output path in it".to_string()))?;
