...
pub(crate) fn driver(input_file: PathBuf, edits_json: Option<PathBuf>, cli_edits: Vec<HeaderEditCli>, output_cfg: OutputOptCli, i2s_version: I2SVersion) -> error_stack::Result<(), CliError> {
    let writer = output_cfg.setup_output(&input_file)
        .change_context_lazy(|| CliError::IoError)?;

    let edits = edits_from_json_and_cli(edits_json.as_deref(), cli_edits)
        .change_context_lazy(|| CliError::BadInput("Could not set up I2S input edits".to_string()))?;
...
impl FromStr for HeaderEditCli {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (param_str, value_str) = s.split_once(",")
            .ok_or_else(|| CliError::BadInput(
                format!("Header edit '{s}' incorrect: must have at least one comma")
            ))?;

        let param = param_str.parse::<usize>()
            .map_err(|e| CliError::BadInput(
                format!("Header edit '{s}' incorrect: value before the comma must be a number ({e})")
            ))?;

        if param < 1 {
            return Err(CliError::BadInput(
                format!("Header edit '{s}' incorrect: parameter number cannot be < 1")
            ));
        }

        Ok(Self { parameter: param, value: value_str.to_string() })
    }
}

pub(crate) fn edits_from_json_and_cli(json_path: Option<&Path>, cli_edits: Vec<HeaderEditCli>) -> error_stack::Result<I2SInputModifcations, CliError> {
