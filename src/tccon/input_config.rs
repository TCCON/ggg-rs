...
use crate::{
    collation::{CollationError, CollationPrefixer},
    error::BodyError,
    utils::{self, get_ggg_path, parse_window_name, FileBuf, GggError},
};
...
impl TcconWindowPrefixes {
    pub fn new_standard_opt() -> error_stack::Result<Option<Self>, BodyError> {
        let gggpath =
            get_ggg_path().change_context_lazy(|| BodyError::custom("could not get GGGPATH"))?;
        let default_file = gggpath.join("tccon").join("secondary_prefixes.dat");
        if default_file.exists() {
            Ok(Some(TcconWindowPrefixes::new(&default_file)?))
        } else {
    ...
    pub fn new(prefix_file: &Path) -> error_stack::Result<Self, BodyError> {
        let f = std::fs::File::open(prefix_file).change_context_lazy(|| {
        ...
        for (iline, line) in rdr.lines().enumerate() {
            ...
            let start_wn = parts.next().ok_or_else(|| {
                BodyError::could_not_read(
                    "line did not include a starting wavenumber",
                    Some(prefix_file.to_path_buf()),
                    Some(iline + 1),
                    None,
                )
            })?;
            let end_wn = parts.next().ok_or_else(|| {
                BodyError::could_not_read(
                    "line did not include an ending wavenumber",
                    Some(prefix_file.to_path_buf()),
                    Some(iline + 1),
                    None,
                )
            ...
            let start_wn = start_wn.parse::<f32>().change_context_lazy(|| {
                BodyError::could_not_read(
                    "starting wavenumber is not a valid number",
                    Some(prefix_file.to_path_buf()),
                    Some(iline + 1),
                    None,
                )
            })?;
            let end_wn = end_wn.parse::<f32>().change_context_lazy(|| {
                BodyError::could_not_read(
                    "ending wavenumber on is not a valid number",
                    Some(prefix_file.to_path_buf()),
                    Some(iline + 1),
                    None,
                )
...
impl CollationPrefixer for TcconWindowPrefixes {
    ...
    fn get_prefix(&self, window: &str) -> Result<&str, CollationError> {
        let entry = self.get_entry(window).map_err(|e| {
            CollationError::custom(format!("Could not get entry for window '{window}': {e}"))
        })?;
        ...
        if entry.prefix.as_ref().is_some_and(|p| window.starts_with(p)) {
            ...
            Err(CollationError::custom(format!(
                "Window {window} begins with a prefix it should not."
            )))
        } else {
...
fn read_correction_file<'de, T: RowWithKey + serde::de::DeserializeOwned>(
    corr_file: &Path,
) -> Result<IndexMap<String, T>, GggError> {
    ...
    for line in f.lines() {
        let line = line.map_err(|e| GggError::CouldNotRead {
