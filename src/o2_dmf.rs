...
impl O2DmfTimeseries {
    pub(crate) fn new(o2_file: PathBuf, run_dir: &Path) -> error_stack::Result<Self, O2DmfError> {
        ...
        Ok(Self {
            ...
            runlog_timestamps,
            delay_years: 2,
            extrap_basis_years: 5,
        })
    }

    fn read_o2_dmf_file(o2_file: &Path) -> error_stack::Result<(Vec<i32>, Vec<f64>), O2DmfError> {
        let f = std::fs::File::open(o2_file)
            .change_context_lazy(|| O2DmfError::input_not_found(o2_file.to_path_buf()))?;
        ...
        let o2_lines: Vec<String> = f
            ...
            .try_collect()
            .change_context_lazy(|| {
                O2DmfError::custom(format!("error reading {}", o2_file.display()))
            })?;

        let colname_line = o2_lines.get(0).ok_or_else(|| {
            O2DmfError::custom(format!(
                "O2 file {} has no non-comment lines",
                o2_file.display()
            ))
        })?;
        let colnames = colname_line
            .trim()
            .split_ascii_whitespace()
            .map(|s| s.to_string())
            .collect_vec();

        let year_idx = colnames.iter().position(|s| s == "year").ok_or_else(|| {
            O2DmfError::custom(format!(
                "could not find 'year' column in {}",
                o2_file.display()
            ))
        })?;

        let o2_idx = colnames.iter().position(|s| s == "fo2").ok_or_else(|| {
            O2DmfError::custom(format!(
                "could not find 'fo2' column in {}",
                o2_file.display()
            ))
        })?;
        ...
        for (iline, line) in o2_lines[1..].into_iter().enumerate() {
            ...
            let year_str = *parts.get(year_idx).ok_or_else(|| {
                O2DmfError::custom(format!(
                    "data line {} in {} does not contain year",
                    iline + 1,
                    o2_file.display()
                ))
            })?;
            ...
            let dmf_str = *parts.get(o2_idx).ok_or_else(|| {
                O2DmfError::custom(format!(
                    "data line {} in {} does not contain fo2",
                    iline + 1,
                    o2_file.display()
                ))
            })?;
    ...
    fn read_runlog(
        run_dir: &Path,
    ) -> error_stack::Result<HashMap<String, chrono::DateTime<chrono::Utc>>, O2DmfError> {
        ...
        let runlog_path =
            get_runlog_from_col_files(&multiggg_file, run_dir).change_context_lazy(|| {
                O2DmfError::custom("could not get a consistent runlog from the .col file headers")
            })?;
        ...
        for (irec, record) in runlog.into_iter().enumerate() {
            let record = record.change_context_lazy(|| {
                O2DmfError::custom(format!(
                    "error occurred reading data record {} in runlog {}",
                    irec + 1,
                    runlog_path.display()
                ))
            })?;
            ...
            let ts = record.zpd_time().ok_or_else(|| {
                O2DmfError::custom(format!("could not get timestamp for spectrum {specname}"))
            })?;
    ...
    fn interpolate_o2(
    ...
    ) -> error_stack::Result<f64, O2DmfError> {
        let last_year_to_keep = dt.year() - self.delay_years;
        let ilast = self
            .years
            .iter()
            .positions(|&y| y <= last_year_to_keep)
            .last()
            .ok_or_else(|| {
                O2DmfError::custom(format!(
        ...
        let ifirst = self
            .years
            .iter()
            .positions(|&y| y >= first_basis_year)
            .min()
            .ok_or_else(|| {
                O2DmfError::custom(format!(
        ...
        // fit the intercept).
        let input_ts = self.years[ifirst..=ilast]
            .iter()
            .map(|&y| {
                chrono::NaiveDate::from_ymd_opt(y, 7, 1)
                    .unwrap()
                    .and_hms_opt(0, 0, 0)
                    .unwrap()
                    .and_utc()
        ...
        let ts_matrix =
            OMatrix::<f64, nalgebra::Dyn, nalgebra::U2>::from_row_iterator(nyear, input_ts);
        ...
        let res = lstsq::lstsq(&ts_matrix, &dmf_vec, epsilon)
            .map_err(|e| O2DmfError::custom(format!("error fitting O2 DMF trend: {e}")))?;
...
impl O2DmfProvider for O2DmfTimeseries {
    fn header_line(&self) -> String {
        format!(
            "O2 DMF source: interpolated from file {}",
            self.o2_file.display()
        )
    }

    fn o2_dmf(&self, spectrum_name: &str) -> error_stack::Result<f64, O2DmfError> {
        let dt = self.runlog_timestamps.get(spectrum_name).ok_or_else(|| {
            O2DmfError::spectrum_not_found(spectrum_name, "spectrum not found in the runlog")
        })?;
...
pub fn make_boxed_o2_dmf_provider(
    clargs: &O2DmfCli,
    run_dir: &Path,
) -> error_stack::Result<Box<dyn O2DmfProvider>, O2DmfError> {
